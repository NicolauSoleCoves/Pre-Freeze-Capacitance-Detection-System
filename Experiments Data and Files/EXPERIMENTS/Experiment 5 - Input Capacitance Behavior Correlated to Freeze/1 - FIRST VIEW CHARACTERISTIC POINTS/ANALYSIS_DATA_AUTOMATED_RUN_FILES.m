...
baseNamePrefix = 'V24V_d1.5cm_Ac16cm2_Parar_Congelat_V_14';

% Loop through each file
for idx = 1:30
    close all;
    fprintf('\n==== Processing File %d ====\n', idx);
    baseName = sprintf('%s%d', baseNamePrefix, idx);
    fullPath = fullfile(folderPath, baseName);

    if ~isfile([fullPath '.mat'])
        warning('File not found: %s.mat. Skipping.', fullPath);
        continue;
    end

    %% === Load file ===
    load(fullPath);  % Expects: timeStamp_T, temperature, evmTimestamps, C4
    ...
    figure(2);clf;
        ...
        subplot(2,1,2)
            ...
            yyaxis right
            plot(t_C, CV, 'Color', "#A2142F", 'DisplayName', 'Temperature');
            plot(t_f_exo, T_peak, 'ro', 'MarkerFaceColor','r', 'DisplayName', 'Freezing Point (T peak)');
            ylabel('T (°C)');
            ax.YColor = "k";
    ...
    %% === Save Figures ===
    % Define a save folder (same as your data folder or another if preferred)
    saveFolder = fullfile(folderPath, 'Plots');
    if ~exist(saveFolder, 'dir')
        mkdir(saveFolder);
    end
    
    % Create unique filenames using the baseName
    fig1Path = fullfile(saveFolder, [baseName '_MainPlot.png']);
    fig2Path = fullfile(saveFolder, [baseName '_MeanStdPlot.png']);
    
    % Save Figure 1 (active by default after plotting)
    figure(1);
    saveas(gcf, fig1Path); % or exportgraphics(gcf, fig1Path, 'Resolution',300);
    
    % Save Figure 2 (you manually call it)
    figure(2);
    saveas(gcf, fig2Path); % or exportgraphics(gcf, fig2Path, 'Resolution',300);
    
    fprintf('Plots saved to:\n  -> %s\n  -> %s\n', fig1Path, fig2Path);


    fprintf('Saved: %s\n', sprintf('%s%d_analysis.png', baseNamePrefix, idx));
end
